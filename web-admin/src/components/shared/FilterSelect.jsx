...
const FilterSelect = ({ value, onChange, options = [], placeholder = 'Chọn...', className }) => {
    ...
    return (
        <div ref={ref} className={cn('relative', className)}>
            ...
            {open && (
                <div className="absolute top-full left-0 mt-1 z-50 min-w-[160px] w-full bg-card border border-border rounded-lg shadow-elevated py-1 animate-fade-in">
                    {options.map((option) => (
                        <button
                            key={option.value}
                            type="button"
